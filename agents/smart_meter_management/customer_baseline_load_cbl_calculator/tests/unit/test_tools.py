...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE customer_baseline_load_cbl_calculator_data;",
        "DELETE FROM customer_baseline_load_cbl_calculator_data WHERE name=1;",
        "TRUNCATE TABLE customer_baseline_load_cbl_calculator_data;",
        "ALTER TABLE customer_baseline_load_cbl_calculator_data DROP COLUMN x;",
        "INSERT INTO customer_baseline_load_cbl_calculator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM customer_baseline_load_cbl_calculator_data LIMIT 10")
