...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE zero_consumption_anomaly_flag_data;",
        "DELETE FROM zero_consumption_anomaly_flag_data WHERE name=1;",
        "TRUNCATE TABLE zero_consumption_anomaly_flag_data;",
        "ALTER TABLE zero_consumption_anomaly_flag_data DROP COLUMN x;",
        "INSERT INTO zero_consumption_anomaly_flag_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM zero_consumption_anomaly_flag_data LIMIT 10")
