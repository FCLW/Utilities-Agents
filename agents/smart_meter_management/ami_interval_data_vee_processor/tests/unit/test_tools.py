...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE ami_interval_data_vee_processor_data;",
        "DELETE FROM ami_interval_data_vee_processor_data WHERE name=1;",
        "TRUNCATE TABLE ami_interval_data_vee_processor_data;",
        "ALTER TABLE ami_interval_data_vee_processor_data DROP COLUMN x;",
        "INSERT INTO ami_interval_data_vee_processor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM ami_interval_data_vee_processor_data LIMIT 10")
