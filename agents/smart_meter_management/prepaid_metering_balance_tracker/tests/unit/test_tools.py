...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE prepaid_metering_balance_tracker_data;",
        "DELETE FROM prepaid_metering_balance_tracker_data WHERE name=1;",
        "TRUNCATE TABLE prepaid_metering_balance_tracker_data;",
        "ALTER TABLE prepaid_metering_balance_tracker_data DROP COLUMN x;",
        "INSERT INTO prepaid_metering_balance_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM prepaid_metering_balance_tracker_data LIMIT 10")
