...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE meter_firmware_ota_scheduler_data;",
        "DELETE FROM meter_firmware_ota_scheduler_data WHERE name=1;",
        "TRUNCATE TABLE meter_firmware_ota_scheduler_data;",
        "ALTER TABLE meter_firmware_ota_scheduler_data DROP COLUMN x;",
        "INSERT INTO meter_firmware_ota_scheduler_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM meter_firmware_ota_scheduler_data LIMIT 10")
