...
def generate_data():
    ...
    ids = ['AMI-9000', 'AMI-9042', 'AMI-9050']
    records = []
    ...
    for eid in ids:
        ...
        df['consumption_kwh'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'consumption_kwh'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "demand_response_thermostat_setback_trigger"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'consumption_kwh'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'consumption_kwh'] *= 0.2
        ...
        final_df.loc[mask, 'consumption_kwh'] = 0.0
        ...
        final_df.loc[mask, 'consumption_kwh'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['meter_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'consumption_kwh'] = 0.0
