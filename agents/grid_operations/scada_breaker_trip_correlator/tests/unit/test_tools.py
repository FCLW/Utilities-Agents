...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE scada_breaker_trip_correlator_data;",
        "DELETE FROM scada_breaker_trip_correlator_data WHERE name=1;",
        "TRUNCATE TABLE scada_breaker_trip_correlator_data;",
        "ALTER TABLE scada_breaker_trip_correlator_data DROP COLUMN x;",
        "INSERT INTO scada_breaker_trip_correlator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM scada_breaker_trip_correlator_data LIMIT 10")
