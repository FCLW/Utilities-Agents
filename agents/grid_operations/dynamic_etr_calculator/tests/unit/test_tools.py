...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE dynamic_etr_calculator_data;",
        "DELETE FROM dynamic_etr_calculator_data WHERE name=1;",
        "TRUNCATE TABLE dynamic_etr_calculator_data;",
        "ALTER TABLE dynamic_etr_calculator_data DROP COLUMN x;",
        "INSERT INTO dynamic_etr_calculator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM dynamic_etr_calculator_data LIMIT 10")
