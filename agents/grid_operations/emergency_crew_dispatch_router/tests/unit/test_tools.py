...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE emergency_crew_dispatch_router_data;",
        "DELETE FROM emergency_crew_dispatch_router_data WHERE name=1;",
        "TRUNCATE TABLE emergency_crew_dispatch_router_data;",
        "ALTER TABLE emergency_crew_dispatch_router_data DROP COLUMN x;",
        "INSERT INTO emergency_crew_dispatch_router_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM emergency_crew_dispatch_router_data LIMIT 10")
