...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE outage_footprint_topology_mapper_data;",
        "DELETE FROM outage_footprint_topology_mapper_data WHERE name=1;",
        "TRUNCATE TABLE outage_footprint_topology_mapper_data;",
        "ALTER TABLE outage_footprint_topology_mapper_data DROP COLUMN x;",
        "INSERT INTO outage_footprint_topology_mapper_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM outage_footprint_topology_mapper_data LIMIT 10")
