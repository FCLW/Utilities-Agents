...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE vegetation_encroachment_lidar_analyzer_data;",
        "DELETE FROM vegetation_encroachment_lidar_analyzer_data WHERE name=1;",
        "TRUNCATE TABLE vegetation_encroachment_lidar_analyzer_data;",
        "ALTER TABLE vegetation_encroachment_lidar_analyzer_data DROP COLUMN x;",
        "INSERT INTO vegetation_encroachment_lidar_analyzer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM vegetation_encroachment_lidar_analyzer_data LIMIT 10")
