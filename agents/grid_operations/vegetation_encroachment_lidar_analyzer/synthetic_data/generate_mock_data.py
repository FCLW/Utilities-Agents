...
def generate_data():
    ...
    for eid in ids:
        ...
        df['voltage_kv'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'voltage_kv'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "vegetation_encroachment_lidar_analyzer"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'voltage_kv'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'voltage_kv'] *= 0.2
        ...
        final_df.loc[mask, 'voltage_kv'] = 0.0
        ...
        final_df.loc[mask, 'voltage_kv'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['feeder_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'voltage_kv'] = 0.0
