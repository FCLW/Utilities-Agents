...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE flisr_switching_simulator_data;",
        "DELETE FROM flisr_switching_simulator_data WHERE name=1;",
        "TRUNCATE TABLE flisr_switching_simulator_data;",
        "ALTER TABLE flisr_switching_simulator_data DROP COLUMN x;",
        "INSERT INTO flisr_switching_simulator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM flisr_switching_simulator_data LIMIT 10")
