...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE mutual_assistance_resource_allocator_data;",
        "DELETE FROM mutual_assistance_resource_allocator_data WHERE name=1;",
        "TRUNCATE TABLE mutual_assistance_resource_allocator_data;",
        "ALTER TABLE mutual_assistance_resource_allocator_data DROP COLUMN x;",
        "INSERT INTO mutual_assistance_resource_allocator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM mutual_assistance_resource_allocator_data LIMIT 10")
