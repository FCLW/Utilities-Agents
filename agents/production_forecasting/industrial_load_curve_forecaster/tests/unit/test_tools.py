...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE industrial_load_curve_forecaster_data;",
        "DELETE FROM industrial_load_curve_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE industrial_load_curve_forecaster_data;",
        "ALTER TABLE industrial_load_curve_forecaster_data DROP COLUMN x;",
        "INSERT INTO industrial_load_curve_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM industrial_load_curve_forecaster_data LIMIT 10")
