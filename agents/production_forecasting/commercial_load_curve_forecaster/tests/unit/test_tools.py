...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE commercial_load_curve_forecaster_data;",
        "DELETE FROM commercial_load_curve_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE commercial_load_curve_forecaster_data;",
        "ALTER TABLE commercial_load_curve_forecaster_data DROP COLUMN x;",
        "INSERT INTO commercial_load_curve_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM commercial_load_curve_forecaster_data LIMIT 10")
