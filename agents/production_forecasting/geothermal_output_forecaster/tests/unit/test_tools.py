...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE geothermal_output_forecaster_data;",
        "DELETE FROM geothermal_output_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE geothermal_output_forecaster_data;",
        "ALTER TABLE geothermal_output_forecaster_data DROP COLUMN x;",
        "INSERT INTO geothermal_output_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM geothermal_output_forecaster_data LIMIT 10")
