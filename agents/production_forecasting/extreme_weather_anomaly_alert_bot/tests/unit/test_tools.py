...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE extreme_weather_anomaly_alert_bot_data;",
        "DELETE FROM extreme_weather_anomaly_alert_bot_data WHERE name=1;",
        "TRUNCATE TABLE extreme_weather_anomaly_alert_bot_data;",
        "ALTER TABLE extreme_weather_anomaly_alert_bot_data DROP COLUMN x;",
        "INSERT INTO extreme_weather_anomaly_alert_bot_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM extreme_weather_anomaly_alert_bot_data LIMIT 10")
