...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE residential_load_curve_forecaster_data;",
        "DELETE FROM residential_load_curve_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE residential_load_curve_forecaster_data;",
        "ALTER TABLE residential_load_curve_forecaster_data DROP COLUMN x;",
        "INSERT INTO residential_load_curve_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM residential_load_curve_forecaster_data LIMIT 10")
