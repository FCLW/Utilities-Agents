...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE hydro_inflow_snowpack_estimator_data;",
        "DELETE FROM hydro_inflow_snowpack_estimator_data WHERE name=1;",
        "TRUNCATE TABLE hydro_inflow_snowpack_estimator_data;",
        "ALTER TABLE hydro_inflow_snowpack_estimator_data DROP COLUMN x;",
        "INSERT INTO hydro_inflow_snowpack_estimator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM hydro_inflow_snowpack_estimator_data LIMIT 10")
