...
def generate_data():
    ...
    ids = ['PLANT-SOLAR-1', 'PLANT-WIND-2', 'PLANT-HYDRO-3']
    records = []
    ...
    for eid in ids:
        ...
        df['generation_mw'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'generation_mw'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "thermal_plant_outage_availability_tracker"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'generation_mw'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'generation_mw'] *= 0.2
        ...
        final_df.loc[mask, 'generation_mw'] = 0.0
        ...
        final_df.loc[mask, 'generation_mw'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['plant_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'generation_mw'] = 0.0
