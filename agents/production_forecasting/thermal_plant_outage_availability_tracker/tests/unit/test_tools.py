...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE thermal_plant_outage_availability_tracker_data;",
        "DELETE FROM thermal_plant_outage_availability_tracker_data WHERE name=1;",
        "TRUNCATE TABLE thermal_plant_outage_availability_tracker_data;",
        "ALTER TABLE thermal_plant_outage_availability_tracker_data DROP COLUMN x;",
        "INSERT INTO thermal_plant_outage_availability_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM thermal_plant_outage_availability_tracker_data LIMIT 10")
