...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE solar_irradiance_predictor_data;",
        "DELETE FROM solar_irradiance_predictor_data WHERE name=1;",
        "TRUNCATE TABLE solar_irradiance_predictor_data;",
        "ALTER TABLE solar_irradiance_predictor_data DROP COLUMN x;",
        "INSERT INTO solar_irradiance_predictor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM solar_irradiance_predictor_data LIMIT 10")
