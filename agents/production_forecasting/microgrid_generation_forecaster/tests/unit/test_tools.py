...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE microgrid_generation_forecaster_data;",
        "DELETE FROM microgrid_generation_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE microgrid_generation_forecaster_data;",
        "ALTER TABLE microgrid_generation_forecaster_data DROP COLUMN x;",
        "INSERT INTO microgrid_generation_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM microgrid_generation_forecaster_data LIMIT 10")
