...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE power_purchase_agreement_legal_reviewer_data;",
        "DELETE FROM power_purchase_agreement_legal_reviewer_data WHERE name=1;",
        "TRUNCATE TABLE power_purchase_agreement_legal_reviewer_data;",
        "ALTER TABLE power_purchase_agreement_legal_reviewer_data DROP COLUMN x;",
        "INSERT INTO power_purchase_agreement_legal_reviewer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM power_purchase_agreement_legal_reviewer_data LIMIT 10")
