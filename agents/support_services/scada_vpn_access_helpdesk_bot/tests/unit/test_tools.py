...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE scada_vpn_access_helpdesk_bot_data;",
        "DELETE FROM scada_vpn_access_helpdesk_bot_data WHERE name=1;",
        "TRUNCATE TABLE scada_vpn_access_helpdesk_bot_data;",
        "ALTER TABLE scada_vpn_access_helpdesk_bot_data DROP COLUMN x;",
        "INSERT INTO scada_vpn_access_helpdesk_bot_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM scada_vpn_access_helpdesk_bot_data LIMIT 10")
