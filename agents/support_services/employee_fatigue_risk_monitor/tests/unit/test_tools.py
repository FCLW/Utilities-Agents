...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE employee_fatigue_risk_monitor_data;",
        "DELETE FROM employee_fatigue_risk_monitor_data WHERE name=1;",
        "TRUNCATE TABLE employee_fatigue_risk_monitor_data;",
        "ALTER TABLE employee_fatigue_risk_monitor_data DROP COLUMN x;",
        "INSERT INTO employee_fatigue_risk_monitor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM employee_fatigue_risk_monitor_data LIMIT 10")
