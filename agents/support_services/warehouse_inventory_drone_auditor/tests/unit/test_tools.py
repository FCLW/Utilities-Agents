...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE warehouse_inventory_drone_auditor_data;",
        "DELETE FROM warehouse_inventory_drone_auditor_data WHERE name=1;",
        "TRUNCATE TABLE warehouse_inventory_drone_auditor_data;",
        "ALTER TABLE warehouse_inventory_drone_auditor_data DROP COLUMN x;",
        "INSERT INTO warehouse_inventory_drone_auditor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM warehouse_inventory_drone_auditor_data LIMIT 10")
