...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE union_contract_benefits_assistant_data;",
        "DELETE FROM union_contract_benefits_assistant_data WHERE name=1;",
        "TRUNCATE TABLE union_contract_benefits_assistant_data;",
        "ALTER TABLE union_contract_benefits_assistant_data DROP COLUMN x;",
        "INSERT INTO union_contract_benefits_assistant_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM union_contract_benefits_assistant_data LIMIT 10")
