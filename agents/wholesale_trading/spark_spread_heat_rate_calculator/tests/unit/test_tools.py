...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE spark_spread_heat_rate_calculator_data;",
        "DELETE FROM spark_spread_heat_rate_calculator_data WHERE name=1;",
        "TRUNCATE TABLE spark_spread_heat_rate_calculator_data;",
        "ALTER TABLE spark_spread_heat_rate_calculator_data DROP COLUMN x;",
        "INSERT INTO spark_spread_heat_rate_calculator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM spark_spread_heat_rate_calculator_data LIMIT 10")
