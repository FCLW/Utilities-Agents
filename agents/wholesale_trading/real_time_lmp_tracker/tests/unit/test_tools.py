...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE real_time_lmp_tracker_data;",
        "DELETE FROM real_time_lmp_tracker_data WHERE name=1;",
        "TRUNCATE TABLE real_time_lmp_tracker_data;",
        "ALTER TABLE real_time_lmp_tracker_data DROP COLUMN x;",
        "INSERT INTO real_time_lmp_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM real_time_lmp_tracker_data LIMIT 10")
