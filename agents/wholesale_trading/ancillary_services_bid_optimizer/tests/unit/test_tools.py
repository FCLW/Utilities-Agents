...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE ancillary_services_bid_optimizer_data;",
        "DELETE FROM ancillary_services_bid_optimizer_data WHERE name=1;",
        "TRUNCATE TABLE ancillary_services_bid_optimizer_data;",
        "ALTER TABLE ancillary_services_bid_optimizer_data DROP COLUMN x;",
        "INSERT INTO ancillary_services_bid_optimizer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM ancillary_services_bid_optimizer_data LIMIT 10")
