...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE portfolio_value_at_risk_analyzer_data;",
        "DELETE FROM portfolio_value_at_risk_analyzer_data WHERE name=1;",
        "TRUNCATE TABLE portfolio_value_at_risk_analyzer_data;",
        "ALTER TABLE portfolio_value_at_risk_analyzer_data DROP COLUMN x;",
        "INSERT INTO portfolio_value_at_risk_analyzer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM portfolio_value_at_risk_analyzer_data LIMIT 10")
