...
def generate_data():
    ...
    ids = ['ISO-NE-HUB', 'PJM-WEST', 'CAISO-NP15']
    records = []
    ...
    for eid in ids:
        ...
        df['price_usd_mwh'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'price_usd_mwh'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "natural_gas_pipeline_constraint_analyzer"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'price_usd_mwh'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'price_usd_mwh'] *= 0.2
        ...
        final_df.loc[mask, 'price_usd_mwh'] = 0.0
        ...
        final_df.loc[mask, 'price_usd_mwh'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['node_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'price_usd_mwh'] = 0.0
