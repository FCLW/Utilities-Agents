...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE day_ahead_lmp_forecaster_data;",
        "DELETE FROM day_ahead_lmp_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE day_ahead_lmp_forecaster_data;",
        "ALTER TABLE day_ahead_lmp_forecaster_data DROP COLUMN x;",
        "INSERT INTO day_ahead_lmp_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM day_ahead_lmp_forecaster_data LIMIT 10")
