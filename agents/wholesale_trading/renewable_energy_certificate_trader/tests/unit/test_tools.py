...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE renewable_energy_certificate_trader_data;",
        "DELETE FROM renewable_energy_certificate_trader_data WHERE name=1;",
        "TRUNCATE TABLE renewable_energy_certificate_trader_data;",
        "ALTER TABLE renewable_energy_certificate_trader_data DROP COLUMN x;",
        "INSERT INTO renewable_energy_certificate_trader_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM renewable_energy_certificate_trader_data LIMIT 10")
