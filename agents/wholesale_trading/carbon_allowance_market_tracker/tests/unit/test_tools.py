...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE carbon_allowance_market_tracker_data;",
        "DELETE FROM carbon_allowance_market_tracker_data WHERE name=1;",
        "TRUNCATE TABLE carbon_allowance_market_tracker_data;",
        "ALTER TABLE carbon_allowance_market_tracker_data DROP COLUMN x;",
        "INSERT INTO carbon_allowance_market_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM carbon_allowance_market_tracker_data LIMIT 10")
