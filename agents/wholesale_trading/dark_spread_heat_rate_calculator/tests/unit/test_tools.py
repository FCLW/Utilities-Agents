...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE dark_spread_heat_rate_calculator_data;",
        "DELETE FROM dark_spread_heat_rate_calculator_data WHERE name=1;",
        "TRUNCATE TABLE dark_spread_heat_rate_calculator_data;",
        "ALTER TABLE dark_spread_heat_rate_calculator_data DROP COLUMN x;",
        "INSERT INTO dark_spread_heat_rate_calculator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM dark_spread_heat_rate_calculator_data LIMIT 10")
