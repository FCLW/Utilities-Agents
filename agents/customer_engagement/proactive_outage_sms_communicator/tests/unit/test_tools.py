...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE proactive_outage_sms_communicator_data;",
        "DELETE FROM proactive_outage_sms_communicator_data WHERE name=1;",
        "TRUNCATE TABLE proactive_outage_sms_communicator_data;",
        "ALTER TABLE proactive_outage_sms_communicator_data DROP COLUMN x;",
        "INSERT INTO proactive_outage_sms_communicator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM proactive_outage_sms_communicator_data LIMIT 10")
