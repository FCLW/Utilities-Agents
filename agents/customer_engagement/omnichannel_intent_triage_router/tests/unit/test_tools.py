...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE omnichannel_intent_triage_router_data;",
        "DELETE FROM omnichannel_intent_triage_router_data WHERE name=1;",
        "TRUNCATE TABLE omnichannel_intent_triage_router_data;",
        "ALTER TABLE omnichannel_intent_triage_router_data DROP COLUMN x;",
        "INSERT INTO omnichannel_intent_triage_router_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM omnichannel_intent_triage_router_data LIMIT 10")
