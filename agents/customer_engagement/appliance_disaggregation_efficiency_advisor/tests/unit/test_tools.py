...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE appliance_disaggregation_efficiency_advisor_data;",
        "DELETE FROM appliance_disaggregation_efficiency_advisor_data WHERE name=1;",
        "TRUNCATE TABLE appliance_disaggregation_efficiency_advisor_data;",
        "ALTER TABLE appliance_disaggregation_efficiency_advisor_data DROP COLUMN x;",
        "INSERT INTO appliance_disaggregation_efficiency_advisor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM appliance_disaggregation_efficiency_advisor_data LIMIT 10")
