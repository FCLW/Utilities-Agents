...
def generate_data():
    ...
    ids = ['CUST-1000', 'CUST-1025', 'CUST-1050']
    records = []
    ...
    for eid in ids:
        ...
        df['amount_billed'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'amount_billed'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "appliance_disaggregation_efficiency_advisor"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'amount_billed'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'amount_billed'] *= 0.2
        ...
        final_df.loc[mask, 'amount_billed'] = 0.0
        ...
        final_df.loc[mask, 'amount_billed'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['customer_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'amount_billed'] = 0.0
