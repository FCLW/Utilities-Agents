...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE high_bill_weather_correlation_explainer_data;",
        "DELETE FROM high_bill_weather_correlation_explainer_data WHERE name=1;",
        "TRUNCATE TABLE high_bill_weather_correlation_explainer_data;",
        "ALTER TABLE high_bill_weather_correlation_explainer_data DROP COLUMN x;",
        "INSERT INTO high_bill_weather_correlation_explainer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM high_bill_weather_correlation_explainer_data LIMIT 10")
