...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE multi_language_translation_router_data;",
        "DELETE FROM multi_language_translation_router_data WHERE name=1;",
        "TRUNCATE TABLE multi_language_translation_router_data;",
        "ALTER TABLE multi_language_translation_router_data DROP COLUMN x;",
        "INSERT INTO multi_language_translation_router_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM multi_language_translation_router_data LIMIT 10")
