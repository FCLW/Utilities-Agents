...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE new_construction_trenching_guide_data;",
        "DELETE FROM new_construction_trenching_guide_data WHERE name=1;",
        "TRUNCATE TABLE new_construction_trenching_guide_data;",
        "ALTER TABLE new_construction_trenching_guide_data DROP COLUMN x;",
        "INSERT INTO new_construction_trenching_guide_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM new_construction_trenching_guide_data LIMIT 10")
