...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE ev_rate_plan_comparison_guide_data;",
        "DELETE FROM ev_rate_plan_comparison_guide_data WHERE name=1;",
        "TRUNCATE TABLE ev_rate_plan_comparison_guide_data;",
        "ALTER TABLE ev_rate_plan_comparison_guide_data DROP COLUMN x;",
        "INSERT INTO ev_rate_plan_comparison_guide_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM ev_rate_plan_comparison_guide_data LIMIT 10")
