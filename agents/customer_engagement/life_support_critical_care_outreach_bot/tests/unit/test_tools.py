...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE life_support_critical_care_outreach_bot_data;",
        "DELETE FROM life_support_critical_care_outreach_bot_data WHERE name=1;",
        "TRUNCATE TABLE life_support_critical_care_outreach_bot_data;",
        "ALTER TABLE life_support_critical_care_outreach_bot_data DROP COLUMN x;",
        "INSERT INTO life_support_critical_care_outreach_bot_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM life_support_critical_care_outreach_bot_data LIMIT 10")
