...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE electric_vehicle_submeter_billing_processor_data;",
        "DELETE FROM electric_vehicle_submeter_billing_processor_data WHERE name=1;",
        "TRUNCATE TABLE electric_vehicle_submeter_billing_processor_data;",
        "ALTER TABLE electric_vehicle_submeter_billing_processor_data DROP COLUMN x;",
        "INSERT INTO electric_vehicle_submeter_billing_processor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM electric_vehicle_submeter_billing_processor_data LIMIT 10")
