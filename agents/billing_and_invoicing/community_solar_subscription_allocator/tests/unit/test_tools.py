...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE community_solar_subscription_allocator_data;",
        "DELETE FROM community_solar_subscription_allocator_data WHERE name=1;",
        "TRUNCATE TABLE community_solar_subscription_allocator_data;",
        "ALTER TABLE community_solar_subscription_allocator_data DROP COLUMN x;",
        "INSERT INTO community_solar_subscription_allocator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM community_solar_subscription_allocator_data LIMIT 10")
