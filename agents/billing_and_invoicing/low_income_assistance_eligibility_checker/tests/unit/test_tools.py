...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE low_income_assistance_eligibility_checker_data;",
        "DELETE FROM low_income_assistance_eligibility_checker_data WHERE name=1;",
        "TRUNCATE TABLE low_income_assistance_eligibility_checker_data;",
        "ALTER TABLE low_income_assistance_eligibility_checker_data DROP COLUMN x;",
        "INSERT INTO low_income_assistance_eligibility_checker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM low_income_assistance_eligibility_checker_data LIMIT 10")
