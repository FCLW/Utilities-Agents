...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE wholesale_vendor_payment_reconciler_data;",
        "DELETE FROM wholesale_vendor_payment_reconciler_data WHERE name=1;",
        "TRUNCATE TABLE wholesale_vendor_payment_reconciler_data;",
        "ALTER TABLE wholesale_vendor_payment_reconciler_data DROP COLUMN x;",
        "INSERT INTO wholesale_vendor_payment_reconciler_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM wholesale_vendor_payment_reconciler_data LIMIT 10")
