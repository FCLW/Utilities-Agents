...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE estimated_bill_fallback_generator_data;",
        "DELETE FROM estimated_bill_fallback_generator_data WHERE name=1;",
        "TRUNCATE TABLE estimated_bill_fallback_generator_data;",
        "ALTER TABLE estimated_bill_fallback_generator_data DROP COLUMN x;",
        "INSERT INTO estimated_bill_fallback_generator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM estimated_bill_fallback_generator_data LIMIT 10")
