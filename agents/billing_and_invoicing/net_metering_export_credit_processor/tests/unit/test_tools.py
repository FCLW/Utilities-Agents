...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE net_metering_export_credit_processor_data;",
        "DELETE FROM net_metering_export_credit_processor_data WHERE name=1;",
        "TRUNCATE TABLE net_metering_export_credit_processor_data;",
        "ALTER TABLE net_metering_export_credit_processor_data DROP COLUMN x;",
        "INSERT INTO net_metering_export_credit_processor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM net_metering_export_credit_processor_data LIMIT 10")
