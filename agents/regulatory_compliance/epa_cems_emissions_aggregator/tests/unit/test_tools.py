...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE epa_cems_emissions_aggregator_data;",
        "DELETE FROM epa_cems_emissions_aggregator_data WHERE name=1;",
        "TRUNCATE TABLE epa_cems_emissions_aggregator_data;",
        "ALTER TABLE epa_cems_emissions_aggregator_data DROP COLUMN x;",
        "INSERT INTO epa_cems_emissions_aggregator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM epa_cems_emissions_aggregator_data LIMIT 10")
