...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE nerc_cip_cybersecurity_audit_analyzer_data;",
        "DELETE FROM nerc_cip_cybersecurity_audit_analyzer_data WHERE name=1;",
        "TRUNCATE TABLE nerc_cip_cybersecurity_audit_analyzer_data;",
        "ALTER TABLE nerc_cip_cybersecurity_audit_analyzer_data DROP COLUMN x;",
        "INSERT INTO nerc_cip_cybersecurity_audit_analyzer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM nerc_cip_cybersecurity_audit_analyzer_data LIMIT 10")
