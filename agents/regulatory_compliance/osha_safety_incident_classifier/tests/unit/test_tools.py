...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE osha_safety_incident_classifier_data;",
        "DELETE FROM osha_safety_incident_classifier_data WHERE name=1;",
        "TRUNCATE TABLE osha_safety_incident_classifier_data;",
        "ALTER TABLE osha_safety_incident_classifier_data DROP COLUMN x;",
        "INSERT INTO osha_safety_incident_classifier_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM osha_safety_incident_classifier_data LIMIT 10")
