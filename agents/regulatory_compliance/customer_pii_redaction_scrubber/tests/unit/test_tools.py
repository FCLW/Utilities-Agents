...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE customer_pii_redaction_scrubber_data;",
        "DELETE FROM customer_pii_redaction_scrubber_data WHERE name=1;",
        "TRUNCATE TABLE customer_pii_redaction_scrubber_data;",
        "ALTER TABLE customer_pii_redaction_scrubber_data DROP COLUMN x;",
        "INSERT INTO customer_pii_redaction_scrubber_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM customer_pii_redaction_scrubber_data LIMIT 10")
