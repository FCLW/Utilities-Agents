...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE water_usage_discharge_reporter_data;",
        "DELETE FROM water_usage_discharge_reporter_data WHERE name=1;",
        "TRUNCATE TABLE water_usage_discharge_reporter_data;",
        "ALTER TABLE water_usage_discharge_reporter_data DROP COLUMN x;",
        "INSERT INTO water_usage_discharge_reporter_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM water_usage_discharge_reporter_data LIMIT 10")
