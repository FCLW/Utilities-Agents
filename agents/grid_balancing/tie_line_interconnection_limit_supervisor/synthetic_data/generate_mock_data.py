...
def generate_data():
    ...
    ids = ['NODE-100', 'NODE-200', 'SUB-ALPHA']
    records = []
    ...
    for eid in ids:
        ...
        df['frequency_hz'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'frequency_hz'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "tie_line_interconnection_limit_supervisor"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'frequency_hz'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'frequency_hz'] *= 0.2
        ...
        final_df.loc[mask, 'frequency_hz'] = 0.0
        ...
        final_df.loc[mask, 'frequency_hz'] += 15.0 # Premium
