...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE distribution_phase_imbalance_detector_data;",
        "DELETE FROM distribution_phase_imbalance_detector_data WHERE name=1;",
        "TRUNCATE TABLE distribution_phase_imbalance_detector_data;",
        "ALTER TABLE distribution_phase_imbalance_detector_data DROP COLUMN x;",
        "INSERT INTO distribution_phase_imbalance_detector_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM distribution_phase_imbalance_detector_data LIMIT 10")
