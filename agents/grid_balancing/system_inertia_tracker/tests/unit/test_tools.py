...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE system_inertia_tracker_data;",
        "DELETE FROM system_inertia_tracker_data WHERE name=1;",
        "TRUNCATE TABLE system_inertia_tracker_data;",
        "ALTER TABLE system_inertia_tracker_data DROP COLUMN x;",
        "INSERT INTO system_inertia_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM system_inertia_tracker_data LIMIT 10")
