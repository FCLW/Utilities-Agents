...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE under_frequency_load_shedding_simulator_data;",
        "DELETE FROM under_frequency_load_shedding_simulator_data WHERE name=1;",
        "TRUNCATE TABLE under_frequency_load_shedding_simulator_data;",
        "ALTER TABLE under_frequency_load_shedding_simulator_data DROP COLUMN x;",
        "INSERT INTO under_frequency_load_shedding_simulator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM under_frequency_load_shedding_simulator_data LIMIT 10")
