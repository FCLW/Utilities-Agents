...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE battery_storage_discharge_optimizer_data;",
        "DELETE FROM battery_storage_discharge_optimizer_data WHERE name=1;",
        "TRUNCATE TABLE battery_storage_discharge_optimizer_data;",
        "ALTER TABLE battery_storage_discharge_optimizer_data DROP COLUMN x;",
        "INSERT INTO battery_storage_discharge_optimizer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM battery_storage_discharge_optimizer_data LIMIT 10")
