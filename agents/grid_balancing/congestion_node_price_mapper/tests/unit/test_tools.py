...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE congestion_node_price_mapper_data;",
        "DELETE FROM congestion_node_price_mapper_data WHERE name=1;",
        "TRUNCATE TABLE congestion_node_price_mapper_data;",
        "ALTER TABLE congestion_node_price_mapper_data DROP COLUMN x;",
        "INSERT INTO congestion_node_price_mapper_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM congestion_node_price_mapper_data LIMIT 10")
