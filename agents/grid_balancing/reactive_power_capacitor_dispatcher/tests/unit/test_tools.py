...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE reactive_power_capacitor_dispatcher_data;",
        "DELETE FROM reactive_power_capacitor_dispatcher_data WHERE name=1;",
        "TRUNCATE TABLE reactive_power_capacitor_dispatcher_data;",
        "ALTER TABLE reactive_power_capacitor_dispatcher_data DROP COLUMN x;",
        "INSERT INTO reactive_power_capacitor_dispatcher_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM reactive_power_capacitor_dispatcher_data LIMIT 10")
