...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE voltage_sag_swell_mitigator_data;",
        "DELETE FROM voltage_sag_swell_mitigator_data WHERE name=1;",
        "TRUNCATE TABLE voltage_sag_swell_mitigator_data;",
        "ALTER TABLE voltage_sag_swell_mitigator_data DROP COLUMN x;",
        "INSERT INTO voltage_sag_swell_mitigator_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM voltage_sag_swell_mitigator_data LIMIT 10")
