...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE drone_inspection_image_processor_data;",
        "DELETE FROM drone_inspection_image_processor_data WHERE name=1;",
        "TRUNCATE TABLE drone_inspection_image_processor_data;",
        "ALTER TABLE drone_inspection_image_processor_data DROP COLUMN x;",
        "INSERT INTO drone_inspection_image_processor_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM drone_inspection_image_processor_data LIMIT 10")
