...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE derms_capacity_optimizer_data;",
        "DELETE FROM derms_capacity_optimizer_data WHERE name=1;",
        "TRUNCATE TABLE derms_capacity_optimizer_data;",
        "ALTER TABLE derms_capacity_optimizer_data DROP COLUMN x;",
        "INSERT INTO derms_capacity_optimizer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM derms_capacity_optimizer_data LIMIT 10")
