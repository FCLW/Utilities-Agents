...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE maintenance_fleet_dispatch_scheduler_data;",
        "DELETE FROM maintenance_fleet_dispatch_scheduler_data WHERE name=1;",
        "TRUNCATE TABLE maintenance_fleet_dispatch_scheduler_data;",
        "ALTER TABLE maintenance_fleet_dispatch_scheduler_data DROP COLUMN x;",
        "INSERT INTO maintenance_fleet_dispatch_scheduler_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM maintenance_fleet_dispatch_scheduler_data LIMIT 10")
