...
def generate_data():
    ...
    ids = ['TX-5543', 'WTG-01', 'CB-990', 'SUB-ALPHA']
    records = []
    ...
    for eid in ids:
        ...
        df['health_score'] = base_val + diurnal + seasonality + np.random.normal(0, 2, len(df))
    ...
    for idx in anomaly_indices:
        final_df.loc[idx, 'health_score'] *= np.random.choice([0.0, 3.5, 5.0]) # Massive drop or spike
    ...
    agent_id = "wind_turbine_gearbox_vibration_analyzer"
    if agent_id == "transformer_dga_health_monitor":
        ...
        final_df.loc[mask, 'health_score'] += np.linspace(0, 100, mask.sum())
        ...
        final_df.loc[mask, 'health_score'] *= 0.2
        ...
        final_df.loc[mask, 'health_score'] = 0.0
        ...
        final_df.loc[mask, 'health_score'] += 15.0 # Premium
    elif agent_id == "ami_interval_data_vee_processor":
        mask = (final_df['asset_id'] == "AMI-9042") & (final_df['timestamp_column'] >= "2026-08-15 10:00:00+00:00") & (final_df['timestamp_column'] <= "2026-08-15 14:00:00+00:00")
        final_df.loc[mask, 'health_score'] = 0.0
