...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE circuit_breaker_wear_analyzer_data;",
        "DELETE FROM circuit_breaker_wear_analyzer_data WHERE name=1;",
        "TRUNCATE TABLE circuit_breaker_wear_analyzer_data;",
        "ALTER TABLE circuit_breaker_wear_analyzer_data DROP COLUMN x;",
        "INSERT INTO circuit_breaker_wear_analyzer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM circuit_breaker_wear_analyzer_data LIMIT 10")
