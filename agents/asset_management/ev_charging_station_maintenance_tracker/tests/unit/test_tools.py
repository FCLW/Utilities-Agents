...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE ev_charging_station_maintenance_tracker_data;",
        "DELETE FROM ev_charging_station_maintenance_tracker_data WHERE name=1;",
        "TRUNCATE TABLE ev_charging_station_maintenance_tracker_data;",
        "ALTER TABLE ev_charging_station_maintenance_tracker_data DROP COLUMN x;",
        "INSERT INTO ev_charging_station_maintenance_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM ev_charging_station_maintenance_tracker_data LIMIT 10")
