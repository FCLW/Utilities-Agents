...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE spare_parts_inventory_forecaster_data;",
        "DELETE FROM spare_parts_inventory_forecaster_data WHERE name=1;",
        "TRUNCATE TABLE spare_parts_inventory_forecaster_data;",
        "ALTER TABLE spare_parts_inventory_forecaster_data DROP COLUMN x;",
        "INSERT INTO spare_parts_inventory_forecaster_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM spare_parts_inventory_forecaster_data LIMIT 10")
