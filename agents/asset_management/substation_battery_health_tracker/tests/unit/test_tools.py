...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE substation_battery_health_tracker_data;",
        "DELETE FROM substation_battery_health_tracker_data WHERE name=1;",
        "TRUNCATE TABLE substation_battery_health_tracker_data;",
        "ALTER TABLE substation_battery_health_tracker_data DROP COLUMN x;",
        "INSERT INTO substation_battery_health_tracker_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM substation_battery_health_tracker_data LIMIT 10")
