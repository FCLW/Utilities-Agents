...
def test_defensive_sql_guardrails():
    ...
    forbidden_queries = [
        "DROP TABLE transmission_tower_corrosion_analyzer_data;",
        "DELETE FROM transmission_tower_corrosion_analyzer_data WHERE name=1;",
        "TRUNCATE TABLE transmission_tower_corrosion_analyzer_data;",
        "ALTER TABLE transmission_tower_corrosion_analyzer_data DROP COLUMN x;",
        "INSERT INTO transmission_tower_corrosion_analyzer_data VALUES (1);"
    ]
...
def test_bigquery_client_execution(mock_bq_client):
    ...
    result = tool.run("SELECT * FROM transmission_tower_corrosion_analyzer_data LIMIT 10")
